---
title: "Modeling Changes in Microbiome: Focused analysis of specific genera"
output:
...
Note: for Clostridium and Ruminococcus we had to select one out of multiple potential reads due to uncertainty in the algorithm.
We selected the closest match for each by using the Clos. or Rum. that had the most reads. 
The choices we had are shown below
...
The effect of the intervention was in the opposite direction (positive) compared to the effect of time (negative) is Genus:
...
If so, then we have evidence that that genus was significantly affected by the intervention, meaning that as time progresses those in the intervention had their microbiome abundance differentially changed over time.
...
unlist_results <- function(fit){
  ...
  colnames(results.out) <- c("Genus", "Term", colnames(out)[2:5])
  ...
  for(i in 2:length(fit)){
    ...
    colnames(results.i) <- c("Genus", "Term", colnames(out)[2:5])
...
#fit2 <- genus.fit6b$`Fixed Effects`
  ...
  colnames(results.out1) <- c("Genus", "Term", colnames(out1)[2:5])
  colnames(results.out2) <- c("Genus", "Term", colnames(out2)[2:5])
  ...
  for(i in 2:length(fit1)){
    ...
    colnames(results.i) <- c("Genus", "Term", colnames(out1)[2:5])
  ...
  for(i in 2:length(fit2)){
    ...
    colnames(results.i) <- c("Genus", "Term", colnames(out2)[2:5])
...
<!-- model5.results <- unlist_results(genus.fit5$`Fixed Effects`) -->
